...
def create_app(env: str | None = None) -> Flask:
    ...
    # NEW TIER SYSTEM: free, plus, pro, max
...
def _ensure_admin(app):
    ...
    try:
        existing = db.session.execute(select(User).where(User.is_admin == True)).scalar_one_or_none()
        if not existing:
            admin = User(email="admin@osint100x.local", full_name="Administrator", tier="max", is_admin=True)
