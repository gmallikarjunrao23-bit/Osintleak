...
def index():
    ...
    if request.method == "POST":
        ...
        if not _check_limit():
            flash("Daily limit reached. Upgrade your plan.", "danger")
...
def export(fmt):
    ...
    if fmt == "json":
        ...
        resp.headers["Content-Disposition"] = f"attachment; filename=osint.json"
        return resp
    if fmt == "csv":
        output = io.StringIO()
        ...
        for src in result.get("sources", []):
            for rec in src.get("records", []):
                writer.writerow([src["title"], rec.get("full_name",""), rec.get("email",""),
                                 ", ".join(rec.get("phones",[])), rec.get("ip",""),
                                 rec.get("country",""), rec.get("region",""),
                                 ", ".join(rec.get("addresses",[])), rec.get("hash","")])
        resp = make_response(output.getvalue())
        resp.headers["Content-Type"] = "text/csv"
        resp.headers["Content-Disposition"] = f"attachment; filename=osint.csv"
        return resp
    abort(400)
